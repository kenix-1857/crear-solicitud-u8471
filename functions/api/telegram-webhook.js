export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!BOT_TOKEN || !BIN_ID || !API_KEY) {
      return new Response('Missing config: ' + !!BOT_TOKEN + !!BIN_ID + !!API_KEY, { status: 500 });
    }
    ...
    if (!binResp.ok) {
      return new Response('JSONBin GET failed: ' + binResp.status + ' ' + await binResp.text(), { status: 500 });
    }
    ...
    if (!putResp.ok) {
      return new Response('JSONBin PUT failed: ' + putResp.status + ' ' + await putResp.text(), { status: 500 });
    }
